...
func TestRunTestsWithRetry(t *testing.T) {
	...
	if testResult.Status != runner.StatusPassed {
		t.Errorf("runTestsWithRetry(...) testResult.Status = %v, want %v", testResult.Status, runner.StatusPassed)
	}
...
func TestRunTestsWithRetry_TestFailedAfterRetry(t *testing.T) {
	...
	if testResult.Status != runner.StatusFailed {
		t.Errorf("runTestsWithRetry(...) testResult.Status = %v, want %v", testResult.Status, runner.StatusFailed)
	}
...
func TestRunTestsWithRetry_Error(t *testing.T) {
	...
	if testResult.Status != runner.StatusError {
		t.Errorf("runTestsWithRetry(...) testResult.Status = %v, want %v", testResult.Status, runner.StatusError)
	}
